...
ercot_linear_model_data$percent_renew <- stringr::str_extract(ercot_linear_model_data$percent_renew, "^.{4}") %>% 
  as.numeric()
...
```{r}
ercot_linear_model_data_resprice <- ercot_linear_model_data %>% 
  select(-c(average_price_com, average_price_ind, average_price_tot))
...
```{r}
ercot.res.Corr <- cor(ercot_linear_model_data_resprice)
corrplot(ercot.res.Corr, method = 'ellipse')
```
```{r}
ercot.AIC.res <- lm(data = ercot_linear_model_data_resprice, 
                       Mean_Load ~ average_price_res + population + percent_renew + med_income +
                        percent_minority)
...
ercot.residential.regression <- lm(data = ercot_linear_model_data_resprice, 
                       Mean_Load ~ average_price_res + percent_renew)

summary(ercot.residential.regression)
