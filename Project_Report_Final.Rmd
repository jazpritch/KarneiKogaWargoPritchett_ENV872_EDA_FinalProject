...
ggplot(rt_load, aes(x= Price, y= Load_MW)) + 
  ...
  labs(subtitle = "Figure 6", x= "Price ($)", y = "Load (MW)") 
```
...
ggplot(dasr_load, aes(x= Price, y= Load_MW)) + 
  ...
  labs(subtitle = "Figure 7", x= "Price ($)", y = "Load (MW)")
```
...
| Variables                                                   | AIC Value |
|-------------------------------------------------------------|-----------|
| population + percent_renew + med_income + percent_minority  | 178       |
...
Regarding our first research question, whether ERCOT and PJM load growth has changed over time, we concluded that the answer is yes. In the time series analyses in section 4.1, both ERCOT and PJM average monthly load was found to statistically increase over time (p-values \< 0.05), accounting for seasonality trends. Additionally, we found that the growth was not equivalent across the two markets; ERCOT displays higher annual load growth (2.58%) than PJM (0.96%).

The simple linear regressions conducted to assess whether there is a linear relationship between LMP and loads in PJM found that, for both real-time and day-ahead market prices, the explanatory variables of market prices were statistically significant (both p-values \< 0.001), but the R-squared values indicated that these models were poorly fit (0.0023 and 0.0367, respectively). With this evidences as well as the figures (Figures 6 and 7), we can conclude that while market prices are significant factors, there may be other variables that could improve fit, or these relationships may be non-linear altogether.

When analyzing what variables contribute to load growth in the ERCOT market, we found that the most significant factors are average bill prices (averaged across all consuming sectors) and the percentage of renewable penetration, found using Akaike's Information Criterion (AIC). Average bill price and percent renewables were both statistically significant in the multiple linear regression (p-values \< 0.05, 0.001 respectively). The coefficient for average bill price was 1976.77, meaning that for every \$1 increase in average bill price, average annual load increases by 1976.77 MW. For renewable penetration, we found that for every 1% increase in renewable energy generation, average annual load increases by 358.75 MW. The high adjusted R-squared value (0.9616) indicates that 96.16% of variation in average annual load can be attributed to the explanatory variables, signalling a good fit for this model. With this model, we have evidence to reject our null hypothesis that there are no relationships between the explanatory and response variables of our model.
